"""
script to measure EE and ED for degraded central alignments
"""
...
def main():
    ...
    for i in range(211, N):
        for j in range(0, 10):
            print(i, j)
            ed, ee = calculate_ed_ee(halocat, mus[j], 0.99, rbins=rbins)

            # save measurements
            fpath = fpath = PROJECT_DIRECTORY + 'halo_shape_correlations/data/'
            fname = simname + '_' + '{0:.2f}'.format(mus[j]) + '_' + '{0:.2f}'.format(0.99) + '_ed_ee_'+str(i).zfill(4)+'.dat'
