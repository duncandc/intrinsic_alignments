"""
The class defined in this module is
used as an orthogonal mix-in class to supplement the behavior of
the analytic profile and velocity models.
The result of using `MonteCarloGalAnisotropicProf` as an orthogonal mix-in class
is a composite class that can be used to generate Monte Carlo realizations
of the full anisotropic phase space distribution of galaxies within their halos.
"""
...
from halotools.empirical_models.model_helpers import custom_spline, call_func_table
...
__all__ = ['MonteCarloGalAnisotropicProf']


class MonteCarloGalAnisotropicProf(object):
    r""" Orthogonal mix-in class used to turn an analytical
    phase space model (e.g., `~halotools.empirical_models.NFWPhaseSpace`)
    into a class that can generate the Anisotropic phase space distribution
